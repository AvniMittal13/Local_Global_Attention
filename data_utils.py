...
def parse_image(crop_path, labels_csv, label_column):  # read image from filename and load it to a tensor
    ...
    def proc(filename):
        ...
        for idx in [0, 120, 240]:
        # name = tf.strings.split(parts[-1], '.')[0]

            image = tf.io.read_file(filename + "/" + video_name + "/" + "frame_" + idx)
        ...
        return images, bboxes, label_text
...
def training_preprocess(table_label):
    def proc(images, bboxes, label):
        ...
        return final_images, final_faces, label
...
def get_train_dataset():
    ...
    labels_csv  = pd.read_csv(config.train_label_csv)
    label_column = labels_csv.columns[config.label_column]
...
def parse_image_test(crop_path, labels_csv, label_column):  # read image from filename and load it to a tensor
...
def get_eval_dataset(kind='test'):
    ...
    if kind == 'train':
        ...
        image_path = config.test_images
        crop_path = config.test_crop
        labels_csv  = pd.read_csv(config.test_label_csv)
    elif kind == 'val':
        image_path = config.val_images
        crop_path = config.val_crop
        labels_csv  = pd.read_csv(config.val_label_csv)
    else:
        raise ValueError('Wrong type of dataset ("train". "val" or "test" is acceptable)')
    ...
    dataset_test = list_ds_test.map(parse_image_test(crop_path, labels_csv, label_column))
